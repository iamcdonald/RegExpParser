...
export const matchers = {
  ...
  '\\\\c(\\w)'    : ([text, content]) => new Meta(types.CONTROL, content, text),
  '\\\\(\\d{3})'  : ([text, content]) => new Meta(types.OCTAL, content, text),
  '\\\\n'         : ([text]) => new Meta(types.NEW_LINE, text, text),
  '\\\\r'         : ([text]) => new Meta(types.RETURN, text, text),
  '\\\\t'         : ([text]) => new Meta(types.TAB, text, text),
  '\\\\b'         : ([text]) => new Meta(types.WORD_BOUNDARY, text, text),
  '\\\\B'         : ([text]) => new Meta(types.NON_WORD_BOUNDARY, text, text)
}
